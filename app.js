...
function renderMemberView(){
  ...
  document.getElementById('mem-body').innerHTML=
    ...
    '<p style="font-size:.82rem;color:var(--muted);margin:-.35rem 0 .9rem;line-height:1.55">Connect with fellow Friends of Biodiversity. Tap a member to view their profile, or follow them directly. Listed by influence (most followers first).</p>'+
    '<div id="members-directory" class="members-dir"><p style="font-size:.85rem;color:var(--muted)">Loading members…</p></div>'+
    // Announcements
...
/* ═══ DISCOVER MEMBERS DIRECTORY — browse & follow influential members ═══ */
async function populateMembersDirectory(){
  ...
  // Influential members: active members (not self, not admins), most followers first
  const list=MEMBERS
    .filter(m=>m.id!==currentUser.id&&m.role==='member'&&m.status==='active')
    .sort((a,b)=>(b.followers_count||0)-(a.followers_count||0))
    .slice(0,12);
  if(!list.length){el.innerHTML='<p style="font-size:.85rem;color:var(--muted)">No other members to show yet — check back as the community grows.</p>';return}
    ...
    return '<div class="mdir-card">'+
      '<div class="mdir-top" onclick="viewMemberProfile(\''+m.id+'\')">'+
        ...
        '<div class="mdir-meta">'+
          ...
          '<div class="mdir-tier">'+esc(td.label)+' · '+(m.followers_count||0)+' follower'+((m.followers_count||0)===1?'':'s')+'</div>'+
        '</div>'+
      '</div>'+
